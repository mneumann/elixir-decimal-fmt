defmodule DecimalFmtTest do
  ...
  test "it converts string into decimal representation" do
    auto_assert {[1], []} <- DecimalFmt.DecimalRepr.from_string("1")
    auto_assert {[1], [0]} <- DecimalFmt.DecimalRepr.from_string("1.0")
    auto_assert {[1], [0, 1, 0]} <- DecimalFmt.DecimalRepr.from_string("1.010")
    auto_assert {[0, 0, 1], [0, 1, 0, 0, 0]} <- DecimalFmt.DecimalRepr.from_string("001.01000")

    auto_assert {[1, 2, 3, 4, 5, 6], [0, 1, 0, 0, 0, 1, 1, 1, 2, 3, 4, 0, 0]} <-
                  DecimalFmt.DecimalRepr.from_string("123456.0100011123400")

    auto_assert_raise BadFunctionError, DecimalFmt.DecimalRepr.from_string("")
    auto_assert_raise BadFunctionError, DecimalFmt.DecimalRepr.from_string("123456.")
    auto_assert_raise BadFunctionError, DecimalFmt.DecimalRepr.from_string(".1")
    auto_assert_raise FunctionClauseError, DecimalFmt.DecimalRepr.from_string("1.2a")
  end
  ...
  test "it formats decimals" do
    ...
    auto_assert "1.12[0]" <-
                  DecimalFmt.fmt(DecimalFmt.DecimalRepr.from_string("1.120"), format_spec)
